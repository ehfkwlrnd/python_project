...
class D365():
    ...
    def createRecord(self, query, data):
        crmRes = requests.post(self.apiEndPoint + query, data=json.dumps(data), headers=self.headers)
        return crmRes

    def updateRecord(self, query, data):
        crmRes = requests.patch(self.apiEndPoint + query, data=json.dumps(data), headers=self.headers)
        return crmRes
